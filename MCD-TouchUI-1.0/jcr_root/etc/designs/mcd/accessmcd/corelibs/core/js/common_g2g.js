...
select: function( event, ui ) {
    if(ui.item.value.indexOf("/")==0 || ui.item.value.indexOf("http")==0 || ui.item.value.indexOf("newwin")==0){
        ...
        logSuggestionClick($('#txt_search').val(),url);
        if(newwin){
...
function setResponsiveAwesomeBarContent(currentPagePath, view, uname, uid, userEmail, currentDesignPath,displayType){
    ...
    /*if($('#li_mysite').length!=0) {    
        var _bookmarksLeft = $('#li_mysite').position().left + 2000;
    ...
    $('#li_mysite').mouseenter(function() {
        ...
        showMyResponsiveSiteRecord(currentPagePath, uid, view,'li_mysite','bookmarksList');        
    });    
...
function showMyResponsiveSiteRecord(URL, userID, view,nav_mysite,navBookmarksList)
{
    var pars = "";  
