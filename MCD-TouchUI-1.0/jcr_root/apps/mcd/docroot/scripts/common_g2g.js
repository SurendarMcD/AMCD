                    ...
                    if(compXmlHttp.status == 200)    
                    {
                        ...
                        if( (currentPagePath == corpPath) || (currentPagePath == usaPath) || (currentPagePath == jpPath) || (currentPagePath == ausPath) || (currentPagePath == nzPath) || (currentPagePath == caPath))
                        {
                            ...
                            if(target !='')
                            {
                                var part = new Array();
                                part = target.split(".");   
                                var amcdTarget =  part[0].toLowerCase() + part[1].toLowerCase();
                                if(part[0].toLowerCase().indexOf('www1') >= 0 || amcdTarget.toLowerCase().indexOf('wwwaccessmcd') >= 0 )    
                                { 
                                    if(redirect=='true') 
                                    {
                                        target = target.replace("www1", "www"); // Uncomment after CutOver 
                                        window.location = target + '?redirect=false' ;
                                     }   
                                }   
                                else 
                                { 
...
function renderNav(tree, id, title,pUrl){ 
    var html="";
    var selchild=null;
    var parentUrl = "";
    var navLabel = title;
    ...
    for(var c in tree.children){
          ...
          var childpath="/content"+child.url;
         

          if(child.label != undefined) {
              if(childpath.indexOf(pUrl)==0 ){
                  ...
                  if(child.childselected || childpath.indexOf(pUrl)==0){
                           html+="<li><a href='" + child.url + "' class='active'>" + child.label + "</a></li>";
                           navLabel = child.label;
                           selchild=child;
                           parentUrl = child.url ; 
...
function validatesearch(link,par1,par2)
{
 ...
 {
  $('#searchalert').hide();
   var formActionn = link + '?qt=' + escape(jQuery.trim($('#txt_search').val())) + '&la=' + par1 + '&mkt=' + par2 ;
