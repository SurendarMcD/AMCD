/*
  * CQ.Ext.ux.DatePickerPlus  Addon
  * CQ.Ext.ux.form.DateFieldPlus  Addon  
  *
  * @author    Marco Wienkoop (wm003/lubber)
  * @copyright (c) 2008-2010, Marco Wienkoop (marco.wienkoop@lubber.de) http://www.lubber.de
  *
  * @class CQ.Ext.ux.DatePickerPlus
  * @extends CQ.Ext.DatePicker
  *
  * v.1.4.1
  *
  * @class CQ.Ext.ux.form.DateFieldPlus
  * @extends CQ.Ext.form.DateField
  *
  
  You need at least CQ.ExtJS 2.0.2 or higher 
  
Also adds CQ.Ext.util.EasterDate
...
* Any amount is greatly appreciated and will help to continue on developing CQ.ExtJS Widgets
*
* You can donate via PayPal to donate@lubber.de

-----------------------------------------------------------------------------------------------------
-- DatePickerPlus CQ.Extension based on 4 contributed extensions from the ext-forum
-- and of course the original Datepicker from the awesome CQ.ExtJS Javascript Library
-----------------------------------------------------------------------------------------------------
 ...
 * @license  licensing of CQ.Ext.ux.DatePickerPlus and CQ.Ext.ux.form.DateFieldPlus depends of the underlying CQ.ExtJS Framework Version
 *
 * If you use CQ.ExtJS <= 2.0.2 CQ.Ext.ux.DatePickerPlus and CQ.Ext.ux.form.DateFieldPlus are licensed under the terms of the
 * LGPL v3
 * License details: http://www.gnu.org/licenses/lgpl.html
 * 
 * If you use CQ.ExtJS >= 2.1 CQ.Ext.ux.DatePickerPlus and CQ.Ext.ux.form.DateFieldPlus are licensed under the terms of the
 * GPL v3
 ...
 * This Addon requires the CQ.ExtJS Library, which is distributed under the terms of the GPL v3 (since CQ.ExtJs V2.1)
...
- checked to work with CQ.ExtJS 3.1.1
...
- checked to work with CQ.ExtJS 3.0.0
- checked to work with CQ.ExtJS 2.3.0
...
- more code optimization for CQ.Ext 3.0 compatibility
- support option "defaultvalue" on datefieldplus
- BUGFIX: setDisabled did not work under CQ.Ext 3.0 (reported by radtad)

v.1.4 Beta [2009/07/03]
- checked to work with CQ.ExtJS 3.0-RC3
- checked to work with CQ.ExtJS 2.2.1
...
- BUGFIX: Array-Cloning was done in a wrong way (reported by lpfLoveCQ.Ext)
...
V1.0 RC3 [2008/04/21]
- checked to work with CQ.ExtJS 2.1
...
- works on Original DateMenu and DateField (CQ.Ext.ux.form.DateFieldPlus) also
...
//set startDay to Sunday by default
    if (typeof startDay === "undefined") {
        startDay=(CQ.Ext.DatePicker?CQ.Ext.DatePicker.prototype.startDay:0);
    }
...
if (!CQ.Ext.util.EasterDate) {
    CQ.Ext.util.EasterDate = function(year, plusDays) {
...
CQ.Ext.namespace('CQ.Ext.ux','CQ.Ext.ux.form');
 ...
 */
CQ.Ext.ux.DatePickerPlus = CQ.Ext.extend(CQ.Ext.DatePicker, {
...
//but remember to include the locale file _AFTER_ datepickerplus !
     ...
     * Wheter TIps should be displayed as CQ.Ext.quicktips or browsercontrolled title-attributes
     */
    useQuickTips : true,
...
//this is accidently called too often in the original (when hovering over monthlabel or bottombar..there is no need to update the cells again and just leaks performance)
    ...
    initComponent : function(){
        CQ.Ext.ux.DatePickerPlus.superclass.initComponent.call(this);
    ...
    activeDateKeyNav: function(direction) {
        if (this.showActiveDate) {
            ...
            var currentGetCell = CQ.Ext.get(this.cellsArray[tmpMonthCell].elements[tmpDayCell]);
//cursor gets out of visible range?
            if (    (tmpDayCell+direction>41 && tmpMonthCell+1>=this.cellsArray.length) ||
                ...
                currentGetCell = CQ.Ext.get(this.cellsArray[tmpMonthCell].elements[tmpDayCell]);
    ...
    handleMouseWheel : function(e){
        if(this.fireEvent("beforemousewheel", this,e) !== false){
            ...
            CQ.Ext.ux.DatePickerPlus.superclass.handleMouseWheel.call(this,e);
...
//calculate the HTML of one month at first to gain some speed when rendering many calendars
        ...
        this.eventEl = CQ.Ext.get(el.firstChild);

        if (this.renderPrevNextButtons) {
            this.leftClickRpt = new CQ.Ext.util.ClickRepeater(this.el.child("td.x-date-left a.npm"), {
            ...
            this.rightClickRpt = new CQ.Ext.util.ClickRepeater(this.el.child("td.x-date-right a.npm"), {
        ...
        if (this.renderPrevNextYearButtons) {
            this.leftYearClickRpt = new CQ.Ext.util.ClickRepeater(this.el.child("td.x-date-left a.npy"), {
            ...
            this.rightYearClickRpt = new CQ.Ext.util.ClickRepeater(this.el.child("td.x-date-right a.npy"), {
        ...
        this.keyNav = new CQ.Ext.KeyNav(this.eventEl, {
            "left" : function(e){
        ...
        for(var xx=0,xxk=this.noOfMonth; xx< xxk; ++xx) {
            cells = CQ.Ext.get(this.id+'-inner-date'+xx).select("tbody td.x-date-date-cell");
            textNodes = CQ.Ext.get(this.id+'-inner-date'+xx).query("tbody td.x-date-date-cell span");
            this.cellsArray[xx] = cells;
            this.textNodesArray[xx] = textNodes;
            if (this.showWeekNumber) {
                weekNumberCells = CQ.Ext.get(this.id+'-inner-date'+xx).select("tbody td.x-date-weeknumber-cell");
                weekNumberTextEls = CQ.Ext.get(this.id+'-inner-date'+xx).select("tbody td.x-date-weeknumber-cell span");                
                this.weekNumberCellsArray[xx] = weekNumberCells;
                this.weekNumberTextElsArray[xx] = weekNumberTextEls;                
                weekNumberHeaderCells = CQ.Ext.get(this.id+'-inner-date'+xx).select("th.x-date-weeknumber-header");
...
//showtoday from CQ.Ext 2.2
        if (this.renderTodayButton || this.showToday) {
            this.todayKeyListener = this.eventEl.addKeyListener(CQ.Ext.EventObject.SPACE, this.spaceKeyPressed,  this);                 
            var today = new Date().dateFormat(this.format);         
            this.todayBtn = new CQ.Ext.Button({
        ...
        if (this.multiSelection && this.renderOkUndoButtons) {
            ...
            this.undoBtn = new CQ.Ext.Button({
...
/*
        if (this.resizable) {       
            var resizer = new CQ.Ext.Resizable(this.el, {
                handles: 'all',
...
//set forcerefresh to false because new date (from old activedate) is already calculated
        ...
        var setCellClass = function(cal, cell,textnode,d){
            ...
            var foundday, eCell = CQ.Ext.get(cell), eTextNode = CQ.Ext.get(textnode), t = d.getTime(), tiptext=false, fvalue;
            cell.title = "";
        ...
        for(var x=0,xk=this.noOfMonth;x<xk;++x) {
            ...
            if ((this.markNationalHolidays || this.eventDates.length>0) && this.useQuickTips) {
                for (var e=0,el=textEls.length;e<el;++e) {
                    CQ.Ext.QuickTips.unregister(textEls[e]);
                }
            }
            ...
            if (x===0 && !this.disableMonthPicker) {
                ...
                monthLabel = CQ.Ext.get(this.id+'-monthLabel' + x);                    
...
/*
                if(CQ.Ext.isOpera && !this.secondPass){
...
*/                          
        ...
        if (this.summarizeHeader && this.noOfMonth > 1) {
            var topHeader = CQ.Ext.get(this.id+'-summarize');
    ...
    handleWeekClick : function(e, t){
        if (!this.disabled) {
            ...
            if ((amount==42 && this.fireEvent("beforemonthclick", this, startmonth,this.lastStateWasSelected) !== false) ||
                ...
                if (!CQ.Ext.EventObject.ctrlKey && this.multiSelectByCTRL) {
    ...
    markDateAsSelected : function(t,fakeCTRL,monthcell,daycell,enableUnselect) {
        var currentGetCell = CQ.Ext.get(this.cellsArray[monthcell].elements[daycell]);
    
        if ((currentGetCell.hasClass("x-date-prevday") || currentGetCell.hasClass("x-date-nextday") ) && this.prevNextDaysView!=="mark") {      
            return false;
        }

        if (this.multiSelection && (CQ.Ext.EventObject.ctrlKey || fakeCTRL)) {
                ...
                if (this.maxSelectionDays === this.preSelectedDates.length) {
                    if (!this.maxNotified)  {
                        if(this.fireEvent("beforemaxdays", this) !== false){
                            CQ.Ext.Msg.alert(this.maxSelectionDaysTitle,this.maxSelectionDaysText.replace(/%0/,this.maxSelectionDays));
                        }
...
//calling update in any case would get too slow on huge multiselect calendars, so set the class for the selected cells manually  (MUCH faster if not calling update() every time!)
    ...
    markSingleDays : function(monthcell,daycell,remove) {
        if(!remove) {
            CQ.Ext.get(this.cellsArray[monthcell].elements[daycell]).addClass("x-date-selected");
            this.preSelectedCells.push((monthcell)+"#"+(daycell));
        }
        else {
            CQ.Ext.get(this.cellsArray[monthcell].elements[daycell]).removeClass("x-date-selected");
    ...
    markGhostDatesAlso : function(monthcell,daycell,remove) {
        if (this.prevNextDaysView=="mark") {
            var currentGetCell = CQ.Ext.get(this.cellsArray[monthcell].elements[daycell]), dayCellDiff;
            if(currentGetCell.hasClass("x-date-prevday") && monthcell>0) {
                dayCellDiff = (5-Math.floor(daycell/7))*7;
                if(CQ.Ext.get(this.cellsArray[monthcell-1].elements[daycell+dayCellDiff]).hasClass("x-date-nextday")) {
                    dayCellDiff-=7;
                }
                ...
                if (daycell<14) {
                    ...
                    dayCellDiff = 28;
                    if(!CQ.Ext.get(this.cellsArray[monthcell+1].elements[daycell-dayCellDiff]).hasClass("x-date-prevday")) {
                        dayCellDiff=35;
                    }
    ...
    removeAllPreselectedClasses : function() {
        for (var e=0,el=this.preSelectedCells.length;e<el;++e) {                                                
            ...
            CQ.Ext.get(this.cellsArray[position[0]].elements[position[1]]).removeClass("x-date-selected");
        }
        this.preSelectedDates = [];
        this.preSelectedCells = [];
    },
    ...
    handleDateClick : function(e, t){
        ...
        e.stopEvent();
        var tp = CQ.Ext.fly(t.parentNode);
    ...
    //mark left ghostdates aswell
                        testCell = CQ.Ext.get(this.cellsArray[tmpMonthCell].elements[tmpDayCell]);
    ...
    spaceKeyPressed: function(e) {
        var ctrlfaker = (((!CQ.Ext.EventObject.ctrlKey && !this.multiSelectByCTRL) || CQ.Ext.EventObject.shiftKey) && this.multiSelection ? true:false);
        if (!this.disabled && this.shiftSpaceSelect == CQ.Ext.EventObject.shiftKey && this.showActiveDate) {
            var adCell = this.activeDateCell.split("#");
            var tmpMonthCell = parseInt(adCell[0],10);
            var tmpDayCell = parseInt(adCell[1],10);
...
/* this is needed to get it displayed in a panel correctly, it is called several times...*/ 
    setSize: CQ.Ext.emptyFn
    
});
CQ.Ext.reg('datepickerplus', CQ.Ext.ux.DatePickerPlus);  


/*
To use DatepickerPlus in menus and datefields, DateItem and datefield needs to be rewritten. This way CQ.Ext.DateMenu stays original and by supplying new config item usePickerPlus:true will use the datepickerplus insted of the original picker. 
*/

    
if (parseInt(CQ.Ext.version.substr(0,1),10)>2) {
//ext 3.0       
    CQ.Ext.menu.DateItem = CQ.Ext.ux.DatePickerPlus;
    CQ.Ext.override(CQ.Ext.menu.DateMenu,{
        initComponent: function(){
            this.on('beforeshow', this.onBeforeShow, this);
            if(this.strict = (CQ.Ext.isIE7 && CQ.Ext.isStrict)){
                this.on('show', this.onShow, this, {single: true, delay: 20});
            }
            var PickerWidget = (this.initialConfig.usePickerPlus ? CQ.Ext.ux.DatePickerPlus : CQ.Ext.DatePicker);
            CQ.Ext.apply(this, {
                plain: true,
                showSeparator: false,
                items: this.picker = new PickerWidget(CQ.Ext.apply({
                    internalRender: this.strict || !CQ.Ext.isIE,
...
//ext 2.x
    CQ.Ext.menu.DateItem = function(config){
        if (config && config.usePickerPlus) {
            CQ.Ext.menu.DateItem.superclass.constructor.call(this, new CQ.Ext.ux.DatePickerPlus(config), config);   //NEW LINE          
        }
        else {
            CQ.Ext.menu.DateItem.superclass.constructor.call(this, new CQ.Ext.DatePicker(config), config);
        }
...
//this breaks in ext 3.0 (CQ.Ext.menu.Adapter and CQ.Ext.menu.DateItem do not exist in ext 3.0 anymore)
    CQ.Ext.extend(CQ.Ext.menu.DateItem, CQ.Ext.menu.Adapter,{
        // private
...
if (CQ.Ext.form && CQ.Ext.form.DateField) {
    CQ.Ext.ux.form.DateFieldPlus = CQ.Ext.extend(CQ.Ext.form.DateField, {
        usePickerPlus: true,
        showWeekNumber: true,
        noOfMonth : 1,
        noOfMonthPerRow : 3,
        ...
        onTriggerClick : function(){
            ...
            if(!this.menu){
                this.menu = new CQ.Ext.menu.DateMenu({
...
//do this only once!                    
            ...
            CQ.Ext.apply(this.menu.picker,  {
                ...
                showToday : this.showToday, //from CQ.Ext 2.2
                format : this.format,
...
//bypass setValue validation on CQ.Ext.DateField
                CQ.Ext.form.DateField.superclass.setValue.call(this, value);
            }
            else {
                CQ.Ext.form.DateField.superclass.setValue.call(this, this.formatDate(this.parseDate(date)));                   
            }
        },
        ...
        validateValue : function(value){
            if (this.multiSelection){
                ...
                for (var e=0,el=values.length;e<el;++e) {                                             
                    if (!CQ.Ext.ux.form.DateFieldPlus.superclass.validateValue.call(field, values[e])) {
                        isValid = false;
                    }
                }
                return isValid;
                ...
                return CQ.Ext.ux.form.DateFieldPlus.superclass.validateValue.call(this, value);
            }         
        },

        getValue : function() {
            if (this.multiSelection) {
                var value = CQ.Ext.form.DateField.superclass.getValue.call(this);
                var field = this;                   
                var values = value.split(this.multiSelectionDelimiter);
                var dates = [];
                for (var e=0,el=values.length;e<el;++e) {                                             
                ...
                return CQ.Ext.ux.form.DateFieldPlus.superclass.getValue.call(this);
            }
        },          
        ...
        onRender:function() {
            ...
            CQ.Ext.ux.form.DateFieldPlus.superclass.onRender.apply(this, arguments);
...
//              this.name =  (typeof this.name==="undefined"?this.id+this.submitFormatAddon:(this.name==this.id?this.name+this.submitFormatAddon:this.name));       
            ...
            if(this.tooltip){
                if(typeof this.tooltip == 'object'){
                    CQ.Ext.QuickTips.register(CQ.Ext.apply({
                          target: this.trigger
                    }, this.tooltip));
                } else {
        ...
        onDisable: function(){
            CQ.Ext.ux.form.DateFieldPlus.superclass.onDisable.apply(this, arguments);
            if(this.hiddenField) {
        ...
        onEnable: function(){
            CQ.Ext.ux.form.DateFieldPlus.superclass.onEnable.apply(this, arguments);
            if(this.hiddenField) {
        ...
        formatHiddenDate : function(date){
            return CQ.Ext.isDate(date) ? CQ.Ext.util.Format.date(date, this.submitFormat) : date;
        },
    ...
    CQ.Ext.reg('datefieldplus', CQ.Ext.ux.form.DateFieldPlus);
}
