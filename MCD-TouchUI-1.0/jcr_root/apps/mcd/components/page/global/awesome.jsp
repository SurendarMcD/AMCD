...
<%!    
    ...
    public String getLinkHtml(String linksData, String emailHoverText, String getLinkHoverText, String bookmarksHoverText, String currentDesignPath, String language, String view, String mcdAudience, String feedbackViewType,HttpServletRequest request){
        ...
        if(links.length>1 && (!"".equals(links[0]))) {      
           ...
           //     outStream.append("<a class='sharebookmarksAction' title='" + bookmarksHoverText + "' href='javascript:void(0)' ><img alt='" + bookmarksHoverText + "' src='" + currentDesignPath + "/images/action_register.gif'></a>");
                ...
                outStream.append("<a id='mySiteLinksList' href='javascript:void(0)' >" + links[1] + "<img src='/images/downArrow.png'/> </a>");
...
try {
    ...
    log.error("awesome.jsp Error " + e.getMessage()); 
}
...
<%
//STALE CACHE WARNING
//FOR SLIM NAV ROLLOUT
//Erik Wannebo 5/18/2014
if(!(view.equals("au") || view.equals("nz"))){

%>
<h2><FONT COLOR=RED><b>YOU ARE NOT SEEING THE LATEST ACCESSMcD UPDATES.<br>Please close all browser sessions and re-open AccessMcD in a new browser.</b></FONT></h2><BR>
<%
}
%>  
...
<form name="searchForm" id="searchForm" action="javascript:validatesearch('<%=searchLink %>','<%=language %>','<%=view %>','<%=inthe %>')">   
   ...
   <div>
    ...
    <div align="center" style="color: rgb(153, 0, 0); display: none;clear:both;" id="searchalert">
             <%= langText.get("Please enter keyword(s) to search") %></div>
   ...
   <%=mcdAudienceText %>
     ...
     <li class="bullet-sptr">&bull;</li>   
                       ...
                       %>   
                                 ...
                                 <li class="viewSwitcher" id ="switcher" style="cursor:pointer">
                                   <a href="javascript:void(0)"> 
                                     <span>
                                            <%=viewSwitcherText%><img src="/images/downArrow.png"/> 
                                     </span>
       ...
       <!--       <li class="bullet-sptr">&bull;</li>   -->
            ...
            <%
            for(i=0; i<((linksData.length )/2); i++)
            {   
            %>          
                 <%=getLinkHtml(linksData[i].getString(), emailHoverText, getLinkHoverText, bookmarksHoverText, currentDesign.getPath(), language, view, mcdAudience, feedbackViewType,request) %> 
            <%                
                if((i + 1)<(linksData.length )/2) {
            %>
                 <li class="bullet-sptr">&bull;</li>
            <%
                }                
