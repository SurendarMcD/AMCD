<%--
  ...
  //wei - modified to print dynamic data for Category Store in the /apps/mcd/widgets/files/pci/form/PCIEntryPanel.js on 11/02/2010
...
<%
     ...
     if (pciViews.size() == 0) {
         //out.println("Please define the category list from Design");
         out.println("[['','Please define the category list from Design','']]");
     } else {
    ...
    for (int i=0; i<pcicategoryEntries.length; i++) {
       // out.println(pcicategoryEntries[i] + "...");
     ...
     for (int j=0; j< pciAL.size(); j++) {
         ...
         if(pciCategory!=null && pciCategory.length>0) {
             if (pciCategory[1].indexOf("'") != -1) {
                 pciCategory[1] = pciCategory[1].replace("'","\\'");
             }
             
             if (j == pciAL.size() - 1) 
                 tempStr = "['"+pciCategory[0]+"','"+pciCategory[1]+"','"+pciCategory[2]+"']";
             else 
                 tempStr = "['"+pciCategory[0]+"','"+pciCategory[1]+"','"+pciCategory[2]+"'],";
                 
             sb.append(tempStr);
              //out.println("[['30001','Atlanta Region Headlines','US']]");       
              //out.println("[["+pciCategory[0]+",'"+pciCategory[1]+"','"+pciCategory[2]+"']]");
         }
      }
