...
function updateInterface(bAfterResults){
    ...
    if(reporttype==null || reporttype==" "){
        ...
        //$("#*help").hide();
        $("[id$=help]").hide();
        $("#"+reporttype+"help").show();
    }
    

    
    if("markets| ".indexOf(reporttype)>-1){
        $("#divSearchMarket").hide();
        $("select[name='searchMkt']").val('ALL');
    }else{
        $("#divSearchMarket").show();
    }
    if("clicks".indexOf(reporttype)>-1){
        $("#divClicks").show();
    }else{
        $("#divClicks").hide();
    } 
    if("queries".indexOf(reporttype)>-1){
        $("#divQueries").show();
    }else{
        $("#divQueries").hide();
    } 
    ...
    if("terms|topclicks|topsuggestions|clicks|queries|sessions".indexOf(reporttype)>-1){
        
        $("#divLimit").show();
    }else{
        $("#divLimit").hide();
    }
    if("|topclicks|topsuggestions|markets|terms".indexOf("|"+reporttype)>-1){
        $("#divRole").show();
    }else{
        $("#divRole").hide();
    }
    ...
    //terms|topclicks|clicks|queries|markets|sessions|qleffectiveness|trending;
...
public void displayReport(javax.servlet.jsp.JspWriter out,HttpServletRequest cqReq,org.apache.sling.api.scripting.SlingScriptHelper sling,boolean isAdmin)throws IOException{
   ...
   // if(!endDate.equals(""))out.println(endDate);
    ...
    //out.println("<br>");
    //out.println("<span id='divHelpTextTitle' onclick=\"$('#divHelpText').toggle();\" >");
    out.println("<span id='divHelpTextTitle' >");
    out.println("<B><font color=blue>Report Type Descriptions:</font></B>");
    out.println("</span>");
    out.println("<a target='new' href='/content/dam/accessmcd/na/us/National Department/Services And Support/Content Manager Library/Search_Report_Utility.docx'>Link To Report User Guide</a><br>");
    out.println("<div id='divHelpText'>");
    out.println("<span id='termshelp'><B>Top Queries:</B> Report of most frequent queries<br></span>");
    out.println("<span id='topclickshelp'><B>Top Clicks:</B> Report of most clicked-on search results. Includes CQ pages, CQ page attachments, DMC documents or CQ DAM documents<br></span>");
    out.println("<span id='querieshelp'><B>Queries:</B>  Displays which queries resulted in clicks on a given page or site<br></span>");
    out.println("<span id='clickshelp'><B>Clicks:</B> Displays which search results were clicked on for a given query <br></span>");
    out.println("<span id='marketshelp'><B>Search Market Usage:</B> Report of monthly search queries/users by Search Market (AMCD view)<br></span>");
    out.println("<span id='trendinghelp'><B>Trending:</B> Display of queries with a recent increase in activity</span>");
    out.println("</div>");
    out.println("<br>");
    ...
    out.println("<OPTION value=\"terms\" "+ (reportType.equals("terms")?"SELECTED":"")+">Top Queries</OPTION>");
        ...
        out.println("<OPTION value=\"topclicks\" "+ (reportType.equals("topclicks")?"SELECTED":"")+">Top Clicks</OPTION>");
        out.println("<OPTION value=\"queries\" "+ (reportType.equals("queries")?"SELECTED":"")+">Queries</OPTION>");
        out.println("<OPTION value=\"clicks\" "+ (reportType.equals("clicks")?"SELECTED":"")+">Clicks</OPTION>");
        out.println("<OPTION value=\"topsuggestions\" "+ (reportType.equals("topsuggestions")?"SELECTED":"")+">Top Search Suggestions</OPTION>");
    ...
    out.println("</div>");
        ...
        out.println("<div id='divQueries'>");
        out.println("<B>Click URL:</B><INPUT type=\"text\" size=\"100\" id=\"docURL\" name=\"docURL\" value=\""+docURL+"\">");
        out.println("<br>");
         out.println("<b>Show clicks for all child pages:</b><INPUT type=checkbox name=\"bChildPages\" "+ (bChildPages?"CHECKED":"")+">");
        out.println("<br>");    
         out.println("</div>");
        
        out.println("<div id='divClicks'>");
        out.println("<B>Query:</B><INPUT type=\"text\" size=\"100\" id=\"clickQuery\" name=\"clickQuery\" value=\""+clickQuery+"\"> ");
        out.println("<br>");   
        out.println("<b>Match Exact Query Only:</b><INPUT type=checkbox name=\"bExactQuery\" "+ (bExactQuery?"CHECKED":"")+">");
        out.println("<br>"); 
        out.println("</div>");
    ...
    */
    ...
    if(!reportType.equals("")){
        ...
        if(reportType.equals("topclicks")){
                 displayTopClickReport(out, conn, searchMkt, startDate, endDate, bArchive, bIncludeRole, nbrTerms);
        }
        if(reportType.equals("topsuggestions")){
                 displayTopSearchSuggestionsReport(out, conn, searchMkt, startDate, endDate, bArchive, bIncludeRole, nbrTerms);
        }
        if(reportType.equals("queries")){
                 displayDocumentQueriesReport(out, conn, searchMkt, startDate, endDate, bArchive, bIncludeRole, nbrTerms,docURL,clickQuery,bExactQuery,bChildPages);
        }
        
        if(reportType.equals("clicks")){
                 displayDocumentQueriesReport(out, conn, searchMkt, startDate, endDate, bArchive, bIncludeRole, nbrTerms,docURL,clickQuery,bExactQuery,bChildPages);
        }
...
if(isAdmin){
    ...
    if(reportType.equals("topclicks")&& bIncludeRole)tbsortcolumn="1";
     if(reportType.equals("topsuggestions")&& bIncludeRole)tbsortcolumn="1";
    /*
    if(reportType.equals("markets")){
        if(bIncludeRole){
            tbsortcolumn="2";
    ...
    */
    ...
    if(reportType.equals("queries"))out.println("$(\"#rawdata2\").tablesorter({sortList: [["+tbsortcolumn+",1]]});");
    if(reportType.equals("clicks"))out.println("$(\"#rawdata2\").tablesorter({sortList: [["+tbsortcolumn+",1]]});");  
    out.println("});");
...
public void displayTopSearchSuggestionsReport(javax.servlet.jsp.JspWriter out, Connection conn, String searchMkt, String startDate, String endDate, boolean bArchive, boolean bIncludeRole, String nbrTerms) throws IOException{
    ...
    try{        
        ...
        "where srch_click_ts > TO_DATE('"+startDate+"','MM/dd/yyyy') "+
        "and TRUNC(srch_click_ts) <= TO_DATE('"+endDate+"','MM/dd/yyyy') " +
        "and srch_click_qt_tx like 'ss | " +
        (searchMkt.equals("ALL")?"":searchMkt+" ")+ 
        "%' " +
        "group by "+(bIncludeRole?"substr(srch_click_qt_tx,0,INSTR(srch_click_qt_tx, '|', -1)),":"")+"srch_click_url_tx "+
        "order by count(*) desc";
        //out.println("SQL:"+searchSql);        
...
public void displayDocumentQueriesReport(javax.servlet.jsp.JspWriter out, Connection conn, String searchMkt, String startDate, String endDate, boolean bArchive, boolean bIncludeRole, String nbrTerms, String docURL, String clickQuery, boolean bExactQuery,boolean bChildPages) throws IOException{
    TreeMap tmDocs=new TreeMap();
    long totalDocs=0;
    DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy");

    if(bChildPages && docURL.lastIndexOf(".")>docURL.lastIndexOf("/")){
    ...
    if(bClickQuery){
        out.println("Clicks for mkt|role|query:"+clickQuery+"<br><br>");
    }else{
        out.println("Search queries resulting in "+(bChildPages?"clicks within site":"clicks on")+":<a href='"+docURL+(bChildPages?".html":"")+"'>"+docURL+(bChildPages?".html":"")+"</a><br><br>"); 
    }
    try{        
        out.println("<TABLE id='rawdata' class='tablesorter'>");
        out.println("<thead><tr style='font-weight:bold'>");
        ...
        if(bChildPages||bClickQuery)out.println("<th>Click URL</th>");
        
        out.println("</tr></thead><tbody>");
        
        String searchSql = "select LOWER(srch_click_qt_tx),"+((bClickQuery || bChildPages)?"srch_click_url_tx,":"")+" count(*),(avg(srch_click_rslt_nu)+1) " +
        "from SRCH.tbsrch_click " +
        "where srch_click_ts > TO_DATE('"+startDate+"','MM/dd/yyyy') "+
        "and TRUNC(srch_click_ts) <= TO_DATE('"+endDate+"','MM/dd/yyyy') ";
        if(bClickQuery){
          if(bExactQuery){
              searchSql +="and (LOWER(srch_click_qt_tx) "+(bClickQueryWildcard?" like '%| "+clickQuery+"' ":" ='"+clickQuery+"' ");
              searchSql +="or LOWER(srch_click_qt_tx) "+(bClickQueryWildcard?" like 'quicklink:"+clickQuery+"' ":" ='"+clickQuery+"' ")+")";
          }else{
              searchSql +="and LOWER(srch_click_qt_tx) "+(bClickQueryWildcard?" like '%"+clickQuery+"%' ":" ='"+clickQuery+"' ");
          }
        }else{
           if(!searchMkt.equals("ALL")){
                searchSql +="and LOWER(srch_click_qt_tx) like '"+searchMkt+" %' ";
           }
           searchSql+="and srch_click_url_tx"+(bChildPages?" like '"+docURL+"%' ":"='"+docURL+"' ");
        }  
        searchSql+= "group by LOWER(srch_click_qt_tx)"+( (bClickQuery || bChildPages)?",srch_click_url_tx ":" ")+
        "order by count(*) desc";
