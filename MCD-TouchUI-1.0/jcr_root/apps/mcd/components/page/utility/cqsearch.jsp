...
<%--
  ...
  Copy of OOTB CQ Search component

    Erik Wannebo 2/6/2013

  Draws the search form and evaluates a query

--%><%@include file="/libs/foundation/global.jsp" %>
<title>CQ Search Test</title>
<link href="/libs/foundation/components/search/clientlibs/search.css" rel="stylesheet" type="text/css">
<%
%><cq:setContentBundle/><%
    ...
    if(requestSearchPath==null)requestSearchPath="/content/accessmcd/";
    if (searchIn != null) {
...
%><center>
  <form action="${currentPage.path}.cqsearch.html" 
    ...
    <input value="<fmt:message key="searchButtonText"/>" type="submit"/><br>
    <b>Path:</b><select name="path">
    <option value="/content/accessmcd/" <%=(requestSearchPath.equals("/content/accessmcd/")?"SELECTED":"")%>>All</option>
    <option value="/content/accessmcd/corp/" <%=(requestSearchPath.equals("/content/accessmcd/corp/")?"SELECTED":"") %>>Corporate</option>
    <option value="/content/accessmcd/na/us/" <%=(requestSearchPath.equals("/content/accessmcd/na/us/")?"SELECTED":"") %>>US</option>
    <option value="/content/accessmcd/na/mcweb/"<%=(requestSearchPath.equals("/content/accessmcd/na/mcweb/")?"SELECTED":"") %>>McWeb</option>
    </select>
    Show Tags?<input type=checkbox name=showtags <%=(showtags!=null)?"CHECKED":""%> />
...
<c:choose>
  ...
  <c:when test="${empty result.hits}">
    ...
    <c:if test="${result.spellcheck != null}">
      <p><fmt:message key="spellcheckText"/> <a href="<c:url value="${currentPage.path}.cqsearch.html"><c:param name="q" value="${result.spellcheck}"/></c:url>"><b><c:out value="${result.spellcheck}"/></b></a></p>
    </c:if>
    ...
    <fmt:message key="statisticsText">
      ...
      <div class="searchRight">
          ...
          <c:if test="${result.facets.tags.containsHit}">
              <p>Tags</p>
              <c:if test="${fn:length(result.facets.tags.buckets) < 50 }" >
              ...
              <c:if test="${fn:length(result.facets.tags.buckets) > 49 }" >
              <font color=red> More than 50 </font>
              </c:if>
          </c:if>
          <% } %>
      ...
      <c:if test="${fn:length(search.relatedQueries) > 0}">
        ...
        <c:forEach var="rq" items="${search.relatedQueries}">
            <a style="margin-right:10px" href="${currentPage.path}.cqsearch.html?q=${rq}"><c:out value="${rq}"/></a>
        </c:forEach>
      </c:if> 
      <br/>
