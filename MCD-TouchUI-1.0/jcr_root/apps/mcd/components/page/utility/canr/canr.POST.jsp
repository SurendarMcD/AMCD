...
Iterator usergroups=user.memberOf();
 ...
 if(slingRequest.getUserPrincipal().getName().equals("admin")){
  ...
  String nusername = (request.getParameter("nusername")!= null) ? new String(request.getParameter("nusername").getBytes("iso-8859-1"), "UTF-8").trim() : "";
  //out.println("nusername :: " + nusername  + "<br>");
  String nuseremail =  (request.getParameter("nuseremail")!= null) ? request.getParameter("nuseremail").toString().trim() : "";
  //out.println("nuseremail :: " + nuseremail + "<br>");
        ...
        for(int i = 0 ; i < rpage.length ; i++){
            ...
            try{
                ...
                if(pageNode.hasProperty("cq:lastReplicationAction")){   
                    ...
                    if(act.trim().toLowerCase().equals("")){
                        act="-";
                    }
                }
                ...
                out.println("Author Replacement Exception:"+e.getMessage());
                out.println("Author Replacement Eror for :: " + rpage[i]);
                log.error("Author Replacement Exception:"+e.getMessage());
                log.error("Author Replacement Exception:",e);
            }
        } 
    ...
    if(replace.equals("true") && usertype.trim().equalsIgnoreCase("owner"))  
    {
     ...
     for(int i = 0 ; i < rpage.length ; i++)
     {
      ...
      try
      {
              ...
              //ArrayList pageOwnerData=cutil.getSiteOwner(rpage[i],slingRequest.getResourceResolver());
               pauth = username;
          pmail =  useremail;
...
//Below code display the pages for Site Owner or Author owned page based on the request parameter.
    ...
    try
    {
     ...
     {
        if((!username.trim().equals("") && (nameValue.equalsIgnoreCase(username))) || (!useremail.trim().equals("") && (mailValue.equalsIgnoreCase(useremail))) )
        {
         count++;
         if(m >0){
     ...
     if(count==0)
      data = "<div style='display:none;font-size:11px;'>norecordfound</div>"; 
    else 
    {
   %>
   <script>
...
{
    out.println("<div style='display:none;font-size:10px;'>norecordfound</div>");
}    
